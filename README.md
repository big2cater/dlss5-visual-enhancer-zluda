...
>
> 最新发布：**v2026.09.14-v6** —— 修掉 CUDA 层一处静默失败（网络输出为空时曾被当作"成功"返回；现在**每帧**校验输出与输入采样，会报错并让调用方重试，同时不会把暗场误判成故障）、预热尾部不再无声卡住也不误杀慢机（连续 60 分钟无模块完成才触发，且只结束已停止消耗 CPU 的子进程）、GPU 自动识别结果写入 `%TEMP%\dlssnr_gpu_autoconfig.log`；并按全量代码审查修复了并行模式音频导致整段白做、运动上传 `Map()` 空指针崩溃、空白阈值常量笔误，以及 GUI（窗口标题版本号、RDNA 4 提示判据、对比/预览的 ffprobe 不再泄漏且同一输入只探测一次）与随包脚本（`fix_tdr.ps1` 备份带时间戳、备份失败即中止、TdrLevel=0 需确认；`diagnose_gpu.ps1` 不再把 HIP `count=0` 显示为绿色）等条目（详见随包说明）；上一版（v5）含 MMA 配对融合提速（RX 7900 XT @640×360：单帧 87 → 67 ms）、"淡入黑场片头被误判为竞态"的判定修复、空白判定在编码 / 单图 / 处理器三处的同类补齐，以及单图明亮门槛的上调（余量 4.8% → 47%）；并修复 **RDNA4（gfx12）预热必然崩溃**的问题（详见随包说明）。
...
![Release](https://img.shields.io/badge/release-v2026.09.14--v6-brightgreen)
...
### 2. 使用方法
1. 从 [Releases 页面](https://github.com/big2cater/dlss5-visual-enhancer-zluda/releases) 下载最新的发布包（例如 `DLSSNRFilter-v2026.09.14-v6-nodlssnr.zip`）；
...
5. **首次使用请先预热**：点击 **预热缓存** 按钮预热翻译缓存（进度实时显示在日志窗，约 20~40 分钟，仅此一次），状态栏显示"缓存已就绪"后即可开始处理！跳过预热直接开始也可以，程序会主动引导。

> ⚠️ **关于首次运行**：首次使用（以及每次更换程序版本或显卡之后）需要把网络编译为本机缓存，全程约 20~40 分钟。强烈建议先点 **预热缓存** ——进度可见，不会像卡死；编译结果永久保存在本机（绑定显卡），之后每次启动秒级拉起。
...
5. Launch `dlssnr_gui.exe` -- the Qt 6 unified GUI that handles both video
   batches and single-image enhancement (the old single-image window has
   been retired).
6. **Warm up first**: click the warm-up button before the first job -- it
