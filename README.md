...
>
> 最新发布：**v2026.09.14-v5** —— 含 MMA 配对融合提速（RX 7900 XT @640×360：单帧 87 → 67 ms）、"淡入黑场片头被误判为竞态"的判定修复、空白判定在编码 / 单图 / 处理器三处的同类补齐，以及单图明亮门槛的上调（余量 4.8% → 47%）；并修复 **RDNA4（gfx12）预热必然崩溃**的问题（详见随包说明）。
...
![Release](https://img.shields.io/badge/release-v2026.09.14--v5-brightgreen)
...
### 2. 使用方法
1. 从 [Releases 页面](https://github.com/big2cater/dlss5-visual-enhancer-zluda/releases) 下载最新的发布包（例如 `DLSSNRFilter-v2026.09.14-v5-nodlssnr.zip`）；
