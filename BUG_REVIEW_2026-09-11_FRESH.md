...
`4d06992`，原因见 `CMakeLists.txt` 的注释）。下列条目**转为历史记录**，原文保留、不再修复：

- H7 —— 关闭窗口时 `std::_Exit(0)` 把「用户中止」报成成功，含其「缓解」段提到该 exe 不在发布包内。
- H12 —— `main_window.cpp:168-172` 的 `--selftest` 守卫过严（结论「索引本身是对的」仍是对的，但文件已不在）。
- §H 的审查对象行；半精度一节里 `gui/main_window.cpp:71-74` 那一行；复核表中两行
  （`main_window.cpp:174`、`main_window.cpp:596` vs `batch_window.cpp:94`）。
- 第 428 行「构建状态」的目标清单（现为 21 个目标，不含该 exe）；第 386 行引用的 `/XD`
  列表也已随之下线 `dlss5-image-enhancer_autogen` 一项。

**其余条目依然有效**，包括落在共享文件里的那些（`core/`、`dlss_layer/`、`tools/`）、以及
§H 里针对 `dlssnr_gui` 自身的问题，例如 H6（`batch_main.cpp` 缺 job object）—— 删掉的只是那个前端。
